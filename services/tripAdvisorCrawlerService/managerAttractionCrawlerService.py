from .workerCrawlerAttractionService import *

class TAAttractionCrawlerManager(threading.Thread):
    ...
    def run(self):
        ...
        for j in range(gv.total_no_of_worker):
            w_thread = TACrawlerAttractionWorker("W" + str(j), "Worker " + str(j), self.threadID, self.webdriver)
            w_thread.start()
...
def initiate_manager():
    ...
    for i in range(no_of_manager):
        thread = TAAttractionCrawlerManager(i, "Manager " + str(i), gv.urls_que)
        thread.start()
